            ...
            <div className='w-full flex flex-col items-center justify-center'>
                ...
                <div className='w-full flex items-center justify-start lg:justify-center gap-8 py-6 overflow-x-scroll scrollbar-none'>
                    {categories && categories.map((category) => 
                        <div key={category.id} className={` ${filter === category.urlParamName ? "bg-cartNumbg" : "bg-card"} group bg-card w-24 min-2-[94px] h-28 cursor-pointer rounded-lg drop-shadpw-xl flex flex-col gap-3 items-center justify-center duration-150 transition-all ease-in-out hover:bg-red-600`}>
                        <div className="w-10 h-10 rounded-full bg-red-600 group-hover:bg-card flex items-center justify-center">
                            <MdEmojiFoodBeverage className='text-card group-hover:text-textColor text-lg'/>
                        </div>
